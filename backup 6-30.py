...
class Squares(object):
    ...
    def draw(self):
        ...
        if any(pygame.mouse.get_pressed()):
                ...
                print(f'RVal at in gridmap: {game.level.gridmap[w_y][w_x]}')
    ...
    try:
        with open('gridmap1.txt', 'w') as f:
            for i in range(COLS):
                f.write('\n')
                for j in range(ROWS):
                    f.write(str(game.level.gridmap[i][j])) # save gridmap GRIDMAP FILES ARE ROTATED 90 degrees
                    if j != ROWS-1:
                        f.write(",")
        f.close()
    except:print(f'{j} {i}')
